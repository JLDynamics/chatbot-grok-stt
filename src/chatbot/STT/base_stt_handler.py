...
class BaseSTTHandler(BaseHandler[STTIn, STTOut]):
    ...
    def should_process_input(self, item: STTIn) -> bool:
        ...
        # Every STT input is a finalized utterance now, so all of them wait for
        # the revision to settle.
        ...
        if gate_wait_s >= 0.05:
            logger.info(
                "%s: STT input gate waited %.3fs for turn=%s rev=%s latest=%s age=%.3fs queue=%s",
