...
from chatbot.LLM.tool_call.function_call import extract_function_calls_from_text
...
from chatbot.LLM.utils import image_url_to_pil, remove_unspeechable, resolve_auto_language
...
class VisionLanguageModelHandler(BaseLanguageModelHandler):
    ...
    def _load_model(self, model_name: str, device: str, torch_dtype: str, gen_kwargs: dict[str, Any]) -> None:
        ...
        if self.backend == "mlx":
            if not HAS_MLX_VLM:
                raise ImportError('mlx-vlm is required for MLX VLM models. Install with: pip install "chatbot[mlx-lm]"')
