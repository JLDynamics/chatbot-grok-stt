...
from chatbot.arguments_classes.chat_tts_arguments import ChatTTSHandlerArguments
from chatbot.arguments_classes.facebookmms_tts_arguments import FacebookMMSTTSHandlerArguments
from chatbot.arguments_classes.faster_whisper_stt_arguments import (
...
from chatbot.arguments_classes.mlx_audio_whisper_arguments import (
    MLXAudioWhisperSTTHandlerArguments,
)
from chatbot.arguments_classes.paraformer_stt_arguments import ParaformerSTTHandlerArguments
from chatbot.arguments_classes.parakeet_tdt_arguments import (
...
from chatbot.arguments_classes.whisper_stt_arguments import WhisperSTTHandlerArguments
...
def _optional_dependency_error(selection: BackendSelection, exc: BaseException) -> ImportError | None:
    ...
    return ImportError(
        ...
        f'Install them with `pip install "chatbot[{extra}]"`. '
...
def _create_audio_input(context: HandlerContext, _config: Mapping[str, Any]) -> Any:
    handler_class = _load_handler("chatbot.LLM.audio_input_notifier", "AudioInputNotifier")
...
def _create_parakeet(context: HandlerContext, config: Mapping[str, Any]) -> Any:
    handler_class = _load_handler("chatbot.STT.parakeet_tdt_handler", "ParakeetTDTSTTHandler")
    setup_kwargs = {
        **config,
...
def _create_local_llm(backend: Literal["transformers", "mlx-lm"]) -> HandlerFactory:
    def create(context: HandlerContext, config: Mapping[str, Any]) -> Any:
        ...
        handler_class = _load_handler("chatbot.LLM.language_model", class_name)
        return handler_class(
            context.stop_event,
            queue_in=context.queue_in,
            queue_out=context.queue_out,
...
STT_BACKENDS = build_backend_registry(
    ...
    [
        ...
        BackendSpec(
            ...
            FasterWhisperSTTHandlerArguments,
            _simple_handler_factory(
                "chatbot.STT.faster_whisper_handler",
        ...
        BackendSpec(
            ...
            _simple_handler_factory(
                "chatbot.STT.paraformer_handler",
...
LLM_BACKENDS = build_backend_registry(
    ...
    [
        ...
        BackendSpec(
            ...
            ChatCompletionsLanguageModelHandlerArguments,
            _simple_handler_factory(
                "chatbot.LLM.chat_completions_language_model",
...
TTS_BACKENDS = build_backend_registry(
    ...
    [
        ...
        BackendSpec(
            ...
            _simple_handler_factory(
                "chatbot.TTS.facebookmms_handler",
