...
- Every unit of work: `orca orchestration task-create` (Target / Change / Constraints / Ownership / Observable acceptance), then `orca orchestration worker-start --task <id> --worktree new-child --name <short> --agent <WORKER_AGENT from config.sh> --setup run --json` (or `orchestration/dispatch-worker.sh --title T --spec S --name n`).
