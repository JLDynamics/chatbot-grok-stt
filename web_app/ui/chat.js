...
export class ChatView {
  ...
  /** Render a captured image in the transcript.
   *  @param {string} dataUrl @param {string} label */
  _appendHistImage(dataUrl, label) {
    const empty = this._chatHistory.querySelector(".chat-empty");
    if (empty) empty.remove();
    const el = document.createElement("div");
    el.className = "hist-msg tool";
    el.innerHTML = `<div class="hist-role"></div><img class="hist-image" alt="Captured image sent to the model" />`;
    const role = /** @type {HTMLDivElement} */ (el.querySelector(".hist-role"));
  ...
  onToolResult(name, argsJson, output, image) {
    ...
    if (image) {
      ...
      try {
        ...
        if (name === "control_screen" && args.action === "screenshot") label = "Desktop screenshot";
      } catch { /* keep the generic camera label */ }
      this._appendHistImage(image, label);
    }
    this._markUnread();
  }
}
